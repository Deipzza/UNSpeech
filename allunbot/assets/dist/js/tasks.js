...
function add_task(tab, task = null) {
  if (task == null && tab.charAt(tab.length - 1) == "1") 
    var content_task = template_task();
  else if (task != null && tab.charAt(tab.length - 1) == "1") 
    var content_task = template_task_by_information(task);
  else if (task == null && tab.charAt(tab.length - 1) != "1")
    var content_task = templae_task_date(task);
  else if (task != null && tab.charAt(tab.length - 1) != "1")
    var content_task = template_task_date_by_information(task);
...
function template_task_date_by_information(task) {
  var tem_button = [
    '<div class="colapse_button col-1">',
    `<a class="collapse_table" type="button" data-bs-toggle="collapse" data-bs-target="#colapse_${task["_id"]}" aria-expanded="false" aria-controls="collapseWidthExample">`,
    '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-chevron-right" viewBox="0 0 16 16">',
  ...
  var tem_content = [
    ...
    "</div>",
    `<div class="subject_task col-5">${task["subject"]}</div>`,
    '<div class="date_task col-2">',
    task["date"],
    "</div>",
    '<div class="task_notification_time col-2">',
...
function templae_task_date() {
  ...
  var tem_content = [
    '<div class="name_task col-3">',
    '<input type="text" class="form-control">',
    "</div>",
    '<div class="subject_task col-3">',
    '<input type="text" class="form-control">',
    "</div>",
    '<div class="date_task col-2">',
...
function save_task(tab_id) {
  $(`${tab_id}.task_check`).each(async function (check) {
    if ($(this).prop("checked") == true) {
      ...
      var task_date = $(`#${task_id}`).find(".date_task").text();
      var task_notification_time = $(`#${task_id}`)
        .find(".task_notification_time")
        .text();
